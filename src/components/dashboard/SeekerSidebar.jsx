...
const navItems = [
  ...
  { icon: Gear, href: "/dashboard/seeker/settings", label: "Account Settings" },
];
  ...
  const sidebarNav = (
    <div className="flex flex-col h-full justify-between bg-[var(--bg-sidebar)] border-r border-[var(--border-color)] text-[var(--text-primary)] select-none">
      ...
      <div className="flex flex-col">
        <div className="h-16 px-5 flex items-center justify-between border-b border-white/[0.07]">
          <Link href="/dashboard/seeker" className="flex items-center gap-2.5 group">
            ...
            <div className="flex flex-col">
              <span className="text-white font-bold text-sm tracking-tight group-hover:text-[#a198ff] transition-colors">TalentGrid</span>
              <span className="text-[9px] font-mono font-semibold text-emerald-400 -mt-0.5">CANDIDATE SUITE</span>
            </div>
          </Link>
          {mobileOpen && (
      ...
      <div className="p-3 border-t border-white/[0.07] flex flex-col gap-2">
        ...
        {session?.user && (
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-2.5 flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 min-w-0">
              <div className="w-7 h-7 rounded-lg bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 flex items-center justify-center text-xs font-bold shrink-0">
                {(session.user.name || "U")[0].toUpperCase()}
              </div>
              <div className="min-w-0">
                <p className="text-xs font-bold text-white truncate">{session.user.name || "Candidate"}</p>
        ...
        >
          <SignOutIcon className="w-4 h-4 text-neutral-500 group-hover:text-red-400" />
          <span>Sign Out</span>
        </button>
      </div>
    </div>
    ...
    <>
      ...
      <div className="lg:hidden sticky top-0 z-40 bg-[var(--bg-sidebar)] border-b border-[var(--border-color)] px-4 py-3 flex items-center justify-between">
        <Link href="/dashboard/seeker" className="flex items-center gap-2">
          ...
          <span className="text-[9px] text-emerald-400 bg-emerald-500/15 px-2 py-0.5 rounded-full font-bold">Seeker</span>
        </Link>
        <button
