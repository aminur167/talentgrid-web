...
const navItems = [
  ...
  { icon: Person, href: "/dashboard/admin/settings", label: "My Profile" },
      ...
      <div className="flex flex-col">
        <div className="h-16 px-5 flex items-center justify-between border-b border-[var(--border-color)]">
          <Link href="/dashboard/admin" className="flex items-center gap-2.5 group">
            ...
            <div className="flex flex-col">
              <span className="font-bold text-sm tracking-tight group-hover:text-amber-500 transition-colors" style={{ color: "var(--text-primary)" }}>TalentGrid</span>
              <span className="text-[9px] font-mono font-semibold text-amber-500 -mt-0.5">ADMIN ROOT</span>
            </div>
          </Link>
          {mobileOpen && (
        ...
        <nav className="flex flex-col gap-1.5 px-3 py-4">
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <item.icon className="w-4 h-4 shrink-0" style={{ color: active ? "#000000" : "var(--text-muted)" }} />
        ...
        {session?.user && (
          ...
          >
            <div className="flex items-center gap-2 min-w-0">
              <div className="w-7 h-7 rounded-lg bg-amber-500/20 border border-amber-500/30 text-amber-500 flex items-center justify-center text-xs font-bold shrink-0">
                <ShieldCheck className="w-3.5 h-3.5" />
              </div>
              <div className="min-w-0">
                <p className="text-xs font-bold truncate group-hover:text-amber-500 transition-colors" style={{ color: "var(--text-primary)" }}>
    ...
    <>
      ...
      <div className="lg:hidden sticky top-0 z-40 bg-[var(--bg-sidebar)] border-b border-[var(--border-color)] px-4 py-3 flex items-center justify-between">
        <Link href="/dashboard/admin" className="flex items-center gap-2">
          ...
          <span className="text-[9px] text-amber-500 bg-amber-500/15 px-2 py-0.5 rounded-full font-bold">Admin</span>
        </Link>
        <button
